...
////// Emissive:
                ...
                float3 emissive = (_Diffuse_var.rgb*_Diffuse_var.a);
                float3 finalColor = emissive + (_Diffuse_var.rgb*(1.0 - _Diffuse_var.a)*(pow(attenuation,_attenpowah)*round(saturate(pow(0.5*dot(normalDirection,lightDirection)+0.5,lerp(_LightPowerNormal,_LightPower,_VertexAlphaData_var)))))*(_LightColor0.rgb+UNITY_LIGHTMODEL_AMBIENT.rgb));
...
////// Lighting:
                float attenuation = LIGHT_ATTENUATION(i);
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float3 finalColor = (_Diffuse_var.rgb*(1.0 - _Diffuse_var.a)*(pow(attenuation,_attenpowah)*round(saturate(pow(0.5*dot(normalDirection,lightDirection)+0.5,lerp(_LightPowerNormal,_LightPower,_VertexAlphaData_var)))))*(_LightColor0.rgb+UNITY_LIGHTMODEL_AMBIENT.rgb));
