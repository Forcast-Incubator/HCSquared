...
Shader "Shader Forge/PBRfader" {
    Properties {
        ...
        _StartNormalStrength ("StartNormalStrength", Float ) = 1
        ...
        _EndNormalStrength ("EndNormalStrength", Float ) = 1
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float node_5892 = 0.0;
                float node_5937 = 1.0;
                float node_456 = saturate(pow(saturate(((node_5892 + ( (_FadeTex_var.r - _TexRangeMin) * (node_5937 - node_5892) ) / (_TexRangeMax - _TexRangeMin))+((_Switch*_SwitchSlider)*2.0+-1.0))),_FadePow));
                float3 _EndBumpMap_var = UnpackNormal(tex2D(_EndBumpMap,TRANSFORM_TEX(i.uv0, _EndBumpMap)));
                float3 normalLocal = lerp(float3(0.5,0.5,1),normalize(float3(((float2(_BumpMap_var.r,_BumpMap_var.g)*(1.0 - node_456))+(float2(_EndBumpMap_var.r,_EndBumpMap_var.g)*node_456)),(lerp(_BumpMap_var.b,node_5937,node_456)*lerp(node_5937,_EndBumpMap_var.b,node_456)))),lerp(_StartNormalStrength,_EndNormalStrength,node_456));
